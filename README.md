...
Production should set `HULY_AUDIT_LOG_PATH` to a persistent volume. The published container is available as `ghcr.io/marketingdotlimited/huly-mcp:0.50.0-mdl.14`; pin a version tag rather than a moving tag.
...
| `prepare_tool_action` | Previews and binds the exact arguments for a destructive or high-impact Huly tool. Performs no mutation and returns a five-minute single-use approval ID together with the inspectable tool name and arguments required by execute_approved_tool_action. |
