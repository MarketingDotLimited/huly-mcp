...
- Chat and communication: pinned channel/DM messages are covered by locator-backed list/set tools. Request-access and translation return explicit unsupported results until Huly exposes stable server-side contracts. Remaining work includes applets, in-message polls, guest communication settings, and external Gmail/Telegram/Huly Mail surfaces plus provider-specific attachments once compatible packages/APIs are proven.
