...
- Reviewed Effect 4 corpus SHA-256:
  `60c8a36957202d6aea0defb6e9fc042431bb80721dd8c2994dea0d58eb65f88d`
...
| 6,532 | Tool inventory | The HR, guarded-administration, approval, saved-document, package-viability, and split read/write proxy tools are reviewed across native/proxy discovery, CLI routing, and bundled-process fixtures. Destructive execution carries the approval ID, exact tool name, and unchanged arguments so hosts can inspect the target before confirmation. The approval executor is exposed as `execute_approved_tool_action` to invalidate stale host schemas while preserving the reviewed ten-action surface. Existing native ordering is preserved and the new proxy tools are inserted before the legacy mixed executor. |
