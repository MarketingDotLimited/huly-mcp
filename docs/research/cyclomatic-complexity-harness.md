...
([Oxlint rule documentation](https://oxc.rs/docs/guide/usage/linter/rules/eslint/complexity))
...
| --- | --- | --- |
| Oxlint `complexity` | Maintained native rule; per-function failure at the source line; classic/modified variants; fits the primary linter and an isolated `check-all` stage. ([docs](https://oxc.rs/docs/guide/usage/linter/rules/eslint/complexity)) | **Use now.** |
| `cyclomatic-complexity` | Dedicated JS/TS CLI with warning/error thresholds and JSON output. It works, but duplicates parsing and introduces another dependency for a gate Oxlint already supplies. ([official repository](https://github.com/pilotpirxie/cyclomatic-complexity#readme)) | Viable fallback, no advantage here. |
