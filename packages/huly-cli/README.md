...
| `huly attachments add-to-document` | Add an attachment to a document. filePath is resolved on the MCP server host, data is client-local base64 content, and fileUrl is fetched by the MCP server. | `--teamspace`, `--document`, `--filename`, `--content-type`, `--file-path`, `--file-url`, `--data`, `--description`, `--pinned`, `--kind`, `--description-file` |
| `huly attachments add-to-issue` | Add an attachment to an issue. filePath is resolved on the MCP server host, data is client-local base64 content, and fileUrl is fetched by the MCP server. | `--project`, `--identifier`, `--filename`, `--content-type`, `--file-path`, `--file-url`, `--data`, `--description`, `--pinned`, `--kind`, `--description-file` |
...
| `huly attachments read-image` | Read a supported image attachment as multimodal content | — |
...
| `huly boards cards unarchive` | Unarchive Board Card | — |
...
| `huly boards unarchive` | Unarchive Board | — |
...
| `huly channels access request` | Request Channel Access | — |
...
| `huly channels join` | Join Channel | — |
| `huly channels leave` | Leave Channel | — |
...
| `huly channels messages translate` | Translate Chat Message | `--channel`, `--dm` |
| `huly channels threads replies list` | List Thread Replies | `--limit` |
| `huly channels unarchive` | Unarchive Channel | — |
...
| `huly contacts organizations customer make` | Make Organization Customer | — |
...
| `huly issues documents link` | Link a document to an issue | `--project`, `--issue-identifier`, `--teamspace`, `--document` |
| `huly issues documents unlink` | Unlink a document from an issue | `--project`, `--issue-identifier`, `--teamspace`, `--document` |
...
| `huly notifications contexts hide` | Hide Notification Context | — |
...
| `huly notifications contexts unarchive` | Unarchive Notification Context | — |
...
| `huly notifications unarchive` | Unarchive Notification | — |
...
| `huly planner todos complete` | Complete Todo | `--done-on` |
...
| `huly planner todos reopen` | Reopen Todo | — |
| `huly planner todos schedule` | Schedule Todo | — |
| `huly planner todos unschedule` | Unschedule Todo | `--work-slot-id`, `--locator`, `--scope`, `--from` |
...
| `huly recruiting vacancy unarchive` | Unarchive Recruiting Vacancy | — |
