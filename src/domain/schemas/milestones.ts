...
}).annotations({ title: "MilestoneSummary", description: "Milestone summary for list operations" })
...
}).annotations({ title: "Milestone", description: "Full milestone with all fields" })
...
export const ListMilestonesParamsSchema = Schema.Struct({
  ...
  limit: Schema.optional(
    LimitParam.annotations({ description: `Maximum number of milestones to return (default: ${DEFAULT_LIMIT})` })
  )
}).annotations({ title: "ListMilestonesParams", description: "Parameters for listing milestones" })
...
export const GetMilestoneParamsSchema = Schema.Struct({
  project: ProjectIdentifier.annotations({ description: "Project identifier (e.g., 'HULY')" }),
  milestone: MilestoneIdentifier.annotations({ description: "Milestone ID or label" })
}).annotations({ title: "GetMilestoneParams", description: "Parameters for getting a single milestone" })
...
export const CreateMilestoneParamsSchema = Schema.Struct({
  project: ProjectIdentifier.annotations({ description: "Project identifier (e.g., 'HULY')" }),
  label: MilestoneLabel.annotations({ description: "Milestone name/label" }),
  description: Schema.optional(
    Schema.String.annotations({
      description: `Milestone description in markdown. ${HULY_NATIVE_REFERENCE_MARKDOWN_INPUT}`
    })
  ),
  targetDate: Timestamp.annotations({ description: "Target date as Unix timestamp in milliseconds" })
}).annotations({ title: "CreateMilestoneParams", description: "Parameters for creating a milestone" })
...
export const UpdateMilestoneParamsSchema = Schema.Struct({
  project: ProjectIdentifier.annotations({ description: "Project identifier (e.g., 'HULY')" }),
  milestone: MilestoneIdentifier.annotations({ description: "Milestone ID or label" }),
  label: Schema.optional(MilestoneLabel.annotations({ description: "New milestone name/label" })),
  description: Schema.optional(
    clearableText(`New milestone description in markdown. ${HULY_NATIVE_REFERENCE_MARKDOWN_INPUT}`)
  ),
  targetDate: Schema.optional(
    Timestamp.annotations({ description: "New target date as Unix timestamp in milliseconds" })
  ),
  status: Schema.optional(MilestoneStatusSchema.annotations({ description: "New milestone status" }))
})
  .pipe(
...
export const SetIssueMilestoneParamsSchema = Schema.Struct({
  ...
  milestone: Schema.NullOr(MilestoneIdentifier).annotations({ description: "Milestone ID or label (null to clear)" })
}).annotations({ title: "SetIssueMilestoneParams", description: "Parameters for setting milestone on an issue" })
...
export const DeleteMilestoneParamsSchema = Schema.Struct({
  project: ProjectIdentifier.annotations({ description: "Project identifier (e.g., 'HULY')" }),
  milestone: MilestoneIdentifier.annotations({ description: "Milestone ID or label" })
}).annotations({ title: "DeleteMilestoneParams", description: "Parameters for deleting a milestone" })
