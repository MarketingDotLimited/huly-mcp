...
export const ListCardSpacesParamsSchema = Schema.Struct({
  ...
  limit: Schema.optional(
    LimitParam.annotations({ description: `Maximum number of card spaces to return (default: ${DEFAULT_LIMIT})` })
  )
}).annotations({ title: "ListCardSpacesParams", description: "Parameters for listing card spaces" })
...
const ListCardsParamsBase = Schema.Struct({
  ...
  type: Schema.optional(
    MasterTagIdentifier.annotations({ description: "Filter by master tag (card type) name or ID" })
  ),
  ...
  contentSearch: Schema.optional(
    Schema.String.annotations({ description: "Search cards by content (fulltext search)" })
  ),
  limit: Schema.optional(
    LimitParam.annotations({ description: `Maximum number of cards to return (default: ${DEFAULT_LIMIT})` })
  )
})
...
).annotations({ title: "ListCardsParams", description: "Parameters for listing cards in a card space" })
...
}).annotations({ title: "GetCardParams", description: "Parameters for getting a single card" })

export type GetCardParams = Schema.Schema.Type<typeof GetCardParamsSchema>

export const CreateCardParamsSchema = Schema.Struct({
  cardSpace: CardSpaceIdentifier.annotations({ description: "Card space name or ID" }),
  type: MasterTagIdentifier.annotations({ description: "Master tag (card type) name or ID" }),
  title: NonEmptyString.annotations({ description: "Card title" }),
  content: Schema.optional(
    Schema.String.annotations({ description: `Card content in markdown. ${HULY_NATIVE_REFERENCE_MARKDOWN_INPUT}` })
  ),
  parent: Schema.optional(
    CardIdentifier.annotations({ description: "Parent card title or ID (for creating child cards)" })
  )
}).annotations({ title: "CreateCardParams", description: "Parameters for creating a card" })
...
export const UpdateCardParamsSchema = Schema.Struct({
  cardSpace: CardSpaceIdentifier.annotations({ description: "Card space name or ID" }),
  card: CardIdentifier.annotations({ description: "Card title or ID" }),
  title: Schema.optional(NonEmptyString.annotations({ description: "New card title" })),
...
export const DeleteCardParamsSchema = Schema.Struct({
  cardSpace: CardSpaceIdentifier.annotations({ description: "Card space name or ID" }),
  card: CardIdentifier.annotations({ description: "Card title or ID" })
}).annotations({ title: "DeleteCardParams", description: "Parameters for deleting a card" })
