...
type ExactRecurringFrequencyValues = [HulyRecurringFrequency] extends [RecurringFrequencyValue]
  ? [RecurringFrequencyValue] extends [HulyRecurringFrequency]
    ? true
    : never
  : never
...
export const RecurringFrequencySchema = Schema.Literals(RecurringFrequencyValues).annotate({
...
export const CreatableRecurringFrequencySchema = Schema.Literals(CreatableRecurringFrequencyValues).annotate({
  title: "CreatableRecurringFrequency",
  description: `Recurring event frequency supported for creation by Huly's recurrence generator: ${enumValuesDescription(
...
type ExactWeekdayValues = [HulyWeekday] extends [WeekdayValue]
  ? [WeekdayValue] extends [HulyWeekday]
    ? true
    : never
  : never
...
export const WeekdaySchema = Schema.Literals(WeekdayValues).annotate({
  title: "Weekday",
...
const RecurringRuleNonFrequencyFields = {
  endDate: Schema.optional(Timestamp.annotate({ description: "End date for recurrence (timestamp)" })),
  count: Schema.optional(RecurrenceCount.annotate({ description: "Number of occurrences" })),
  interval: Schema.optional(RecurrenceInterval.annotate({ description: "Interval between occurrences" })),
  // Huly stores byDay as string[], but its generator reliably supports weekday abbreviations here.
  byDay: Schema.optional(
    Schema.Array(WeekdaySchema).annotate({
      description:
  ...
  byMonthDay: Schema.optional(
    Schema.Array(MonthDayOrdinal).annotate({
  ...
  byMonth: Schema.optional(
    Schema.Array(MonthIndex).annotate({ description: "Zero-based months (0=January, 11=December)" })
  ),
  bySetPos: Schema.optional(
    Schema.Array(SetPositionOrdinal).annotate({ description: "Position within set (e.g., -1 for last)" })
  ),
  wkst: Schema.optional(WeekdaySchema.annotate({ description: "Week start day" }))
}

export const RecurringRuleSchema = Schema.Struct({
  freq: RecurringFrequencySchema.annotate({ description: "Frequency (DAILY, WEEKLY, MONTHLY, YEARLY, etc.)" }),
  ...RecurringRuleNonFrequencyFields
}).annotate({ title: "RecurringRule", description: "RFC5545 recurring rule" })
...
export const CreatableRecurringRuleSchema = Schema.Struct({
  freq: CreatableRecurringFrequencySchema.annotate({
...
export const ListRecurringEventsParamsSchema = Schema.Struct({
  limit: Schema.optional(
    LimitParam.annotate({ description: `Maximum number of recurring events to return (default: ${DEFAULT_LIMIT})` })
  )
}).annotate({ title: "ListRecurringEventsParams", description: "Parameters for listing recurring events" })
...
export const CreateRecurringEventParamsSchema = Schema.Struct({
  ...
  startDate: Timestamp.annotate({ description: "First occurrence start date/time (timestamp)" }),
  ...
  timeZone: Schema.optional(TimeZoneId.annotate({ description: "Time zone (e.g., 'America/New_York')" })),
  ...
  .annotate({ title: "CreateRecurringEventParams", description: "Parameters for creating a recurring event" })
...
export const ListEventInstancesParamsSchema = Schema.Struct({
  recurringEventId: EventId.annotate({ description: "Recurring event ID" }),
  ...
  limit: Schema.optional(
    LimitParam.annotate({ description: `Maximum number of instances to return (default: ${DEFAULT_LIMIT})` })
  ),
  includeParticipants: Schema.optional(
    Schema.Boolean.annotate({
      description: `Include full participant info (requires extra lookups, default: ${DEFAULT_RECURRING_INSTANCE_PARTICIPANTS_INCLUDED})`
    })
  )
}).annotate({ title: "ListEventInstancesParams", description: "Parameters for listing instances of a recurring event" })

export type ListEventInstancesParams = Schema.Schema.Type<typeof ListEventInstancesParamsSchema>

export const listRecurringEventsParamsJsonSchema = toDraft07JsonSchema(ListRecurringEventsParamsSchema)
export const createRecurringEventParamsJsonSchema = withMutuallyExclusiveFields(
  toDraft07JsonSchema(CreateRecurringEventParamsSchema),
  CALENDAR_TARGET_FIELDS
)
export const listEventInstancesParamsJsonSchema = toDraft07JsonSchema(ListEventInstancesParamsSchema)

export const parseListRecurringEventsParams = Schema.decodeUnknownEffect(ListRecurringEventsParamsSchema)
export const parseCreateRecurringEventParams = Schema.decodeUnknownEffect(CreateRecurringEventParamsSchema)
export const parseListEventInstancesParams = Schema.decodeUnknownEffect(ListEventInstancesParamsSchema)

export const CreateRecurringEventResultSchema = Schema.Struct({ eventId: EventId })
