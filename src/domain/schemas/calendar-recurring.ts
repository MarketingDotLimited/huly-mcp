...
const RecurringRuleNonFrequencyFields = {
  endDate: Schema.optional(Timestamp.annotations({ description: "End date for recurrence (timestamp)" })),
  count: Schema.optional(RecurrenceCount.annotations({ description: "Number of occurrences" })),
  interval: Schema.optional(RecurrenceInterval.annotations({ description: "Interval between occurrences" })),
  ...
  byMonth: Schema.optional(
    Schema.Array(MonthIndex).annotations({ description: "Zero-based months (0=January, 11=December)" })
  ),
  bySetPos: Schema.optional(
    Schema.Array(SetPositionOrdinal).annotations({ description: "Position within set (e.g., -1 for last)" })
  ),
  wkst: Schema.optional(WeekdaySchema.annotations({ description: "Week start day" }))
}

export const RecurringRuleSchema = Schema.Struct({
  freq: RecurringFrequencySchema.annotations({ description: "Frequency (DAILY, WEEKLY, MONTHLY, YEARLY, etc.)" }),
  ...RecurringRuleNonFrequencyFields
}).annotations({ title: "RecurringRule", description: "RFC5545 recurring rule" })
...
export const ListRecurringEventsParamsSchema = Schema.Struct({
  limit: Schema.optional(
    LimitParam.annotations({ description: `Maximum number of recurring events to return (default: ${DEFAULT_LIMIT})` })
  )
}).annotations({ title: "ListRecurringEventsParams", description: "Parameters for listing recurring events" })
...
export const CreateRecurringEventParamsSchema = Schema.Struct({
  ...
  startDate: Timestamp.annotations({ description: "First occurrence start date/time (timestamp)" }),
  ...
  timeZone: Schema.optional(TimeZoneId.annotations({ description: "Time zone (e.g., 'America/New_York')" })),
  ...
  .annotations({ title: "CreateRecurringEventParams", description: "Parameters for creating a recurring event" })
...
export const ListEventInstancesParamsSchema = Schema.Struct({
  recurringEventId: EventId.annotations({ description: "Recurring event ID" }),
  ...
  limit: Schema.optional(
    LimitParam.annotations({ description: `Maximum number of instances to return (default: ${DEFAULT_LIMIT})` })
  ),
