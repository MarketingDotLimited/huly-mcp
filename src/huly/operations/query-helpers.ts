 ...
 */
export const withLookup = <T extends Doc>(options: FindOptions<T> | undefined, lookups: Lookup<T>): FindOptions<T> => ({
  ...options,
  lookup: { ...options?.lookup, ...lookups }
})
...
export const findOneOrFail = <T extends Doc, E>(
  ...
  Effect.flatMap(client.findOne<T>(_class, hulyQuery(query), options), (result) =>
    result !== undefined ? Effect.succeed(result) : Effect.fail(onNotFound())
  )
...
export const findByNameOrId = <T extends Doc>(
  ...
  Effect.flatMap(client.findOne<T>(_class, hulyQuery(primaryQuery), options), (result) =>
    result !== undefined ? Effect.succeed(result) : client.findOne<T>(_class, hulyQuery(fallbackQuery), options)
  )
...
export const findByNameOrIdOrFail = <T extends Doc, E>(
  ...
  Effect.flatMap(findByNameOrId(client, _class, primaryQuery, fallbackQuery, options), (result) =>
    result !== undefined ? Effect.succeed(result) : Effect.fail(onNotFound())
  )
