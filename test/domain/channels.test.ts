...
describe("Channel Domain Schemas", () => {
  describe("ListChannelsParamsSchema", () => {
    ...
    it.effect("parses with all options", () =>
      Effect.gen(function* () {
        const result = yield* parseListChannelsParams({ includeArchived: true, limit: 25 })
  ...
  describe("CreateChannelParamsSchema", () => {
    ...
    it.effect("parses with topic and private", () =>
      Effect.gen(function* () {
        const result = yield* parseCreateChannelParams({ name: "new-channel", topic: "Channel topic", private: true })
  ...
  describe("UpdateChannelParamsSchema", () => {
    ...
    it.effect("parses with update fields", () =>
      Effect.gen(function* () {
        const result = yield* parseUpdateChannelParams({ channel: "general", name: "new-name", topic: "Updated topic" })
        expect(result.channel).toBe("general")
  ...
  describe("ListChannelMessagesParamsSchema", () => {
    ...
    it.effect("parses with limit", () =>
      Effect.gen(function* () {
        const result = yield* parseListChannelMessagesParams({ channel: "general", limit: 100 })
  ...
  describe("SendChannelMessageParamsSchema", () => {
    it.effect("parses valid params", () =>
      Effect.gen(function* () {
        const result = yield* parseSendChannelMessageParams({ channel: "general", body: "Hello everyone!" })
        expect(result.channel).toBe("general")
