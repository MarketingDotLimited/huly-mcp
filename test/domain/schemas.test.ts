...
describe("Domain Schemas", () => {
  ...
  describe("PersonRefSchema", () => {
    ...
    it.effect("parses with all fields", () =>
      Effect.gen(function* () {
        ...
        expect(result).toEqual({ id: "person-123", name: "John Doe", email: "john@example.com" })
      })
    )
  })
  ...
  describe("Process write schemas", () => {
    it.effect("parses start_process params and exposes required JSON schema fields", () =>
      Effect.gen(function* () {
        const parsed = yield* parseStartProcessParams({ process: "Approval", card: "Contract" })
        expect(parsed).toEqual({ process: "Approval", card: "Contract" })
    ...
    it.effect("parses cancel_execution params and exposes required JSON schema fields", () =>
      Effect.gen(function* () {
        const parsed = yield* parseCancelExecutionParams({ execution: "execution-1" })
        expect(parsed).toEqual({ execution: "execution-1" })
  ...
  describe("ListIssuesParamsSchema", () => {
    ...
    it.effect("parses statusCategory as a Huly workflow category", () =>
      Effect.gen(function* () {
        const result = yield* parseListIssuesParams({ project: "HULY", statusCategory: "active" })
    ...
    it.effect("accepts assignee as a display name (not just an email)", () =>
      Effect.gen(function* () {
        const result = yield* parseListIssuesParams({ project: "HULY", assignee: "Braeden Bihag" })
    ...
    it.effect("test-management updates reject minimal params and advertise update-field requirement", () =>
      Effect.gen(function* () {
        ...
        expect(runSchema.anyOf).toEqual(
          expect.arrayContaining([{ required: ["name"] }, { required: ["description"] }, { required: ["dueDate"] }])
        )

        const resultSchema = expectJsonSchemaObject(updateTestResultParamsJsonSchema)
        expect(resultSchema.anyOf).toEqual(
          expect.arrayContaining([{ required: ["status"] }, { required: ["assignee"] }, { required: ["description"] }])
        )
      })
    )
  })
  ...
  describe("AddLabelParamsSchema", () => {
    it.effect("parses valid params", () =>
      Effect.gen(function* () {
        const result = yield* parseAddLabelParams({ project: "HULY", identifier: "HULY-123", label: "bug" })
        expect(result).toEqual({ project: "HULY", identifier: "HULY-123", label: "bug" })
      })
    )
  ...
  describe("ListProjectsParamsSchema", () => {
    ...
    it.effect("parses with all options", () =>
      Effect.gen(function* () {
        const result = yield* parseListProjectsParams({ includeArchived: true, limit: 25 })
        expect(result).toEqual({ includeArchived: true, limit: 25 })
      })
    )
  ...
  describe("ListTeamspacesParamsSchema", () => {
    ...
    it.effect("parses with all options", () =>
      Effect.gen(function* () {
        const result = yield* parseListTeamspacesParams({ includeArchived: true, limit: 25 })
  ...
  describe("ListDocumentsParamsSchema", () => {
    ...
    it.effect("parses with limit", () =>
      Effect.gen(function* () {
        const result = yield* parseListDocumentsParams({ teamspace: "My Docs", limit: 100 })
  ...
  describe("GetDocumentParamsSchema", () => {
    it.effect("parses valid params", () =>
      Effect.gen(function* () {
        const result = yield* parseGetDocumentParams({ teamspace: "My Docs", document: "Getting Started" })
  ...
  describe("CreateDocumentParamsSchema", () => {
    it.effect("parses minimal params", () =>
      Effect.gen(function* () {
        const result = yield* parseCreateDocumentParams({ teamspace: "My Docs", title: "New Document" })
        expect(result.teamspace).toBe("My Docs")
  ...
  describe("DeleteDocumentParamsSchema", () => {
    it.effect("parses valid params", () =>
      Effect.gen(function* () {
        const result = yield* parseDeleteDocumentParams({ teamspace: "My Docs", document: "Old Document" })
        expect(result.teamspace).toBe("My Docs")
  ...
  describe("LogTimeParamsSchema", () => {
    it.effect("parses minimal params", () =>
      Effect.gen(function* () {
        const result = yield* parseLogTimeParams({ project: "TEST", identifier: "TEST-1", value: 30 })
  ...
  describe("GetTimeReportParamsSchema", () => {
    it.effect("parses valid params", () =>
      Effect.gen(function* () {
        const result = yield* parseGetTimeReportParams({ project: "TEST", identifier: "TEST-1" })
        expect(result.project).toBe("TEST")
        expect(result.identifier).toBe("TEST-1")
      })
    )
  ...
  describe("StartTimerParamsSchema", () => {
    it.effect("parses valid params", () =>
      Effect.gen(function* () {
        const result = yield* parseStartTimerParams({ project: "TEST", identifier: "TEST-1" })
        expect(result.project).toBe("TEST")
        expect(result.identifier).toBe("TEST-1")
      })
    )
  ...
  describe("StopTimerParamsSchema", () => {
    it.effect("parses valid params", () =>
      Effect.gen(function* () {
        const result = yield* parseStopTimerParams({ project: "TEST", identifier: "TEST-1" })
        expect(result.project).toBe("TEST")
        expect(result.identifier).toBe("TEST-1")
      })
    )
