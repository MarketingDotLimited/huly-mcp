...
const makeSearchResultDoc = (overrides: {
  ...
  parseSearchResultDoc({
    id: overrides._id,
    doc: { _id: overrides._id, _class: overrides._class, createdOn: overrides.createdOn },
...
const createTestLayer = (
  ...
  const searchFulltextImpl: HulyClientOperations["searchFulltext"] = ((query: SearchQuery, options: SearchOptions) => {
    if (captureArgs) {
...
describe("fulltextSearch", () => {
  it.effect("returns mapped results with correct fields", () =>
    Effect.gen(function* () {
      const docs = [
        makeSearchResultDoc({ _id: "doc-1", _class: "core:class:Doc", title: "First", score: 10, createdOn: 2000 }),
        makeSearchResultDoc({
          _id: "doc-2",
