...
describe("sdk discovery operations", () => {
        ...
        classes: [
          {
            ...
            domain: "tracker",
            attributesCount: 2,
            firstClassToolHints: [{ category: "issues", exampleTools: ["list_issues", "get_issue", "create_issue"] }],
            routingHints: [
              {
                status: "covered",
  ...
  it.effect("uses the schema SDK discovery default limit when no limit is provided", () =>
    Effect.gen(function* () {
      ...
      const classes = Array.from({ length: classCount }, (_, index) =>
        makeClassDoc({ _id: `test:class:Item${index}`, label: `test:class:Item${index}`, domain: "test" })
      )
      ...
      expect(encoded.attributes).toEqual(
        expect.arrayContaining([
          ...
          expect.objectContaining({
            ...
            type: expect.objectContaining({
              kind: "array",
              arrayOf: expect.objectContaining({ kind: "array", arrayOf: expect.objectContaining({ kind: "string" }) })
            })
          })
        ])
      )
  ...
  it.effect("gets attributes from implemented interfaces and interface ancestors", () =>
    Effect.gen(function* () {
      ...
      const classes = [
        makeClassDoc({ _id: timestampInterface, label: "core:interface:Timestamped", kind: ClassifierKind.INTERFACE }),
        makeClassDoc({
        ...
        makeClassDoc({ _id: tracker.class.Issue, label: "tracker:class:Issue", implements: [auditableInterface] })
      ]
      const attributes = [
